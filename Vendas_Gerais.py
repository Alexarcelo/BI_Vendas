...
def gerar_df_metas_vendedor():
    ...
    st.session_state.df_metas_vendedor['Mes_Ano'] = pd.to_datetime(
        st.session_state.df_metas_vendedor['Ano'].astype(str) + '-' + st.session_state.df_metas_vendedor['Mes'].astype(str) + '-01'
    ).dt.to_period('M')
...
def gerar_df_metas():
    ...
    tratar_colunas_numero_df(
        ...
        st.session_state.df_metas.columns
    )

    st.session_state.df_metas['Mes_Ano'] = pd.to_datetime(
        st.session_state.df_metas['Ano'].astype(str) + '-' + st.session_state.df_metas['Mes'].astype(str) + '-01'
    ).dt.to_period('M')
...
def puxar_df_config():
    ...
    tratar_colunas_numero_df(
        ...
        st.session_state.lista_colunas_numero_df_config
    )

    st.session_state.passeios_incluidos = st.session_state.df_config[
        st.session_state.df_config['Configuração']=='Passeios Gráfico Pizza'
    ]['Parâmetro'].tolist()

    st.session_state.combo_luck = st.session_state.df_config[
        st.session_state.df_config['Configuração']=='Passeios Combo Luck'
    ]['Parâmetro'].tolist()

    if st.session_state.base_luck == 'test_phoenix_natal':

        st.session_state.servicos_terceiros = st.session_state.df_config[
            st.session_state.df_config['Configuração']=='Serviços de Terceiros'
        ]['Parâmetro'].tolist()
...
def gerar_df_vendas_final():
    ...
    def gerar_df_vendas_manuais():
        ...
        tratar_colunas_numero_df(
            st.session_state.df_vendas_manuais, 
            st.session_state.lista_colunas_numero_df_vendas_manuais
        )

        tratar_colunas_data_df(
            st.session_state.df_vendas_manuais, 
            st.session_state.lista_colunas_data_df_vendas_manuais
        )
...
def gerar_df_guias_in():
    ...
    st.session_state.df_guias_in = gerar_df_phoenix(
        st.session_state.base_luck, 
        request_select
    )
    ...
    if st.session_state.base_luck == 'test_phoenix_joao_pessoa':
        ...
        substituicao = {
            'RAQUEL - PDV': 'RAQUEL - GUIA', 
            'VALERIA - PDV': 'VALERIA - GUIA', 
            'ROBERTA - PDV': 'ROBERTA - GUIA', 
            'LETICIA - TRANSFERISTA': 'LETICIA - GUIA', 
            'SORAYA - BASE AEROPORTO ': 'SORAYA - GUIA', 
            'SORAYA - TRANSFERISTA': 'SORAYA - GUIA'
        }
...
def gerar_df_paxs_in():
    ...
    st.session_state.df_paxs_in = gerar_df_phoenix(
        st.session_state.base_luck, 
        request_select
    )
    ...
    if st.session_state.base_luck in ['test_phoenix_joao_pessoa', 'test_phoenix_salvador']:  
        ...
        st.session_state.df_paxs_in = pd.merge(
            st.session_state.df_paxs_in, 
            st.session_state.df_metas[
                [
                    'Mes_Ano', 
                    'Paxs_Desc'
...
def colher_periodo_e_setor(col1):
    ...
    with col1:
        ...
        seleciona_setor = st.multiselect(
            'Setor', 
            sorted(lista_setor), 
            default=None, 
            key='seleciona_setor'
        ...
        if st.session_state.base_luck=='test_phoenix_noronha':
            ...
            base_tm = st.multiselect(
                'Base Ticket Médio', 
                [
                    'Paxs Recebidos', 
                    'Número de Serviços'
                ], 
                default='Paxs Recebidos'
            )
...
def filtrar_periodo_dfs_base_luck(data_ini, data_fim, mes_ano_ini, mes_ano_fim):
    ...
    def filtrar_periodo_dfs(data_ini, data_fim, mes_ano_ini, mes_ano_fim):
        ...
        df_vendas = st.session_state.df_vendas_final[
            (st.session_state.df_vendas_final['Data_Venda'] >= data_ini) & 
            (st.session_state.df_vendas_final['Data_Venda'] <= data_fim)
        ].reset_index(drop=True)

        df_paxs_in = st.session_state.df_paxs_in[
            (st.session_state.df_paxs_in['Data_Execucao'] >= data_ini) & 
            (st.session_state.df_paxs_in['Data_Execucao'] <= data_fim)
        ].reset_index(drop=True)

        df_guias_in = st.session_state.df_guias_in[
            (st.session_state.df_guias_in['Data da Escala'] >= data_ini) & 
            (st.session_state.df_guias_in['Data da Escala'] <= data_fim)
        ].reset_index(drop=True)

        df_metas_vendedor = st.session_state.df_metas_vendedor[
            (st.session_state.df_metas_vendedor['Mes_Ano'] >= mes_ano_ini) & 
            (st.session_state.df_metas_vendedor['Mes_Ano'] <= mes_ano_fim)
        ].reset_index(drop=True)
            
        df_metas_setor = st.session_state.df_metas[
            (st.session_state.df_metas['Mes_Ano'] >= mes_ano_ini) & 
            (st.session_state.df_metas['Mes_Ano'] <= mes_ano_fim)
        ].reset_index(drop=True)

        if st.session_state.base_luck=='test_phoenix_natal':

            df_ocupacao_hoteis = st.session_state.df_ocupacao_hoteis[
                (st.session_state.df_ocupacao_hoteis['Mes_Ano'] >= mes_ano_ini) & 
                (st.session_state.df_ocupacao_hoteis['Mes_Ano'] <= mes_ano_fim)
            ].reset_index(drop=True)
            ...
            df_metas_vendedor_setor = st.session_state.df_metas_vendedor_setor[
                (st.session_state.df_metas_vendedor_setor['Mes_Ano'] >= mes_ano_ini) &
                (st.session_state.df_metas_vendedor_setor['Mes_Ano'] <= mes_ano_fim)
            ].reset_index(drop=True)
            
            df_metas_vendedor_canal_vendas = st.session_state.df_metas_vendedor_canal_vendas[
                (st.session_state.df_metas_vendedor_canal_vendas['Mes_Ano'] >= mes_ano_ini) &
                (st.session_state.df_metas_vendedor_canal_vendas['Mes_Ano'] <= mes_ano_fim)
            ].reset_index(drop=True)
    ...
    if st.session_state.base_luck=='test_phoenix_natal':
        ...
        df_vendas, df_paxs_in, df_guias_in, df_metas_vendedor, df_metas_setor, df_metas_vendedor_setor, df_metas_vendedor_canal_vendas = filtrar_periodo_dfs(
            data_ini, 
            data_fim, 
            mes_ano_ini, 
            mes_ano_fim
...
def colher_selecoes_vendedor_canal_hotel(df_vendas, col1):
    ...
    seleciona_canal = col1.multiselect(
        'Canal de Vendas', 
        lista_canal, 
        key='Can_on'
    )

    seleciona_vend = col1.multiselect(
        'Vendedor', 
        lista_vendedor, 
        key='Ven_on'
    )

    seleciona_hotel = col1.multiselect(
        'Hotel', 
        lista_hotel, 
        key='Hot_on'
    )
    ...
    if st.session_state.base_luck=='test_phoenix_natal':
        ...
        filtrar_servicos_terceiros = col1.multiselect(
            'Filtrar Serviços de Terceiros', 
            ['Sim'], 
            default=None
        )
...
def gerar_df_hotel(df_vendas):
    ...
    df_hotel.rename(
        columns=
        {
            'Desconto_Global_Por_Servico': 'Desconto Reserva x Serviços', 
            'Valor_Venda': 'Vendas'
        }, 
        inplace=True
    )

    df_hotel['Hotel'] = np.where(
        ~df_hotel['Estabelecimento_Origem'].str.upper().str.contains('AEROPORTO'), 
        df_hotel['Estabelecimento_Origem'], 
        df_hotel['Estabelecimento_Destino']
    )

    df_hotel = df_hotel.groupby(
        [
            'Mes_Ano',
            'Vendedor', 
            'Hotel'
        ...
        {
            'Vendas': 'sum', 
            'Desconto Reserva x Serviços': 'sum'
        }
    ).reset_index()
...
def ajustar_desconto_global(df_vendas):
    ...
    if st.session_state.base_luck == 'test_phoenix_joao_pessoa':
        ...
        valor_ref = np.where(
            df_vendas['Data_Venda'] >= datetime.date(2024, 12, 1), 
            1000, 
            5000
        )
        ...
        df_vendas['Desconto_Global_Ajustado'] = np.where(
            (
                (df_vendas['Desconto_Global_Por_Servico'].notna()) & 
                (df_vendas['Desconto_Global_Por_Servico'] < valor_ref) & 
                (df_vendas['Servico'] != 'EXTRA')
            ), 
...
def gerar_df_vendas_agrupado(df_vendas, df_metas_vendedor, df_guias_in, df_paxs_in, df_metas_setor, df_ocupacao_hoteis=None, df_metas_vendedor_setor=None):
    ...
    def inserindo_meta_vendedor_periodo(df_metas_vendedor, df_vendas_agrupado):
        ...
        df_metas_vendedor_periodo = df_metas_vendedor.groupby(
            'Vendedor', 
            as_index=False
        )['Meta_Mes'].mean()
        ...
        if st.session_state.base_luck == 'test_phoenix_salvador':
            ...
            df_metas_vendedor_setor_periodo = df_metas_vendedor_setor.groupby(
                [
                    'Vendedor', 
                    'Setor'
                ], 
            ...
            df_metas_vendedor_setor_periodo.rename(
                columns={
                    'Meta_Mes': 'Meta_Vendedor_Setor'
                }, 
                inplace=True
            )
            ...
            df_vendas_agrupado = pd.merge(
                df_vendas_agrupado, 
                df_metas_vendedor_setor_periodo, 
                on=[
                    'Vendedor', 
                    'Setor'
                ], 
    ...
    df_vendas_agrupado = inserindo_paxs_in_vendedor(
        df_vendas_agrupado, 
        df_guias_in
    )
    ...
    if st.session_state.base_luck in ['test_phoenix_joao_pessoa', 'test_phoenix_salvador', 'test_phoenix_recife']:
...
def gerar_soma_vendas_tm_vendas_desconto_paxs_recebidos(df_vendas_agrupado):
    ...
    def escolher_paxs_ref(row):
        ...
        if (
            row['Setor']=='Transferista' or 
            (
                st.session_state.base_luck in ['test_phoenix_natal', 'test_phoenix_recife'] and 
                row['Setor']=='Guia'
            )
        ):
            ...
            st.session_state.base_luck == 'test_phoenix_natal' and 
            row['Setor']=='Desks'
        ):
    ...
    def calcular_ticket_medio_realizado_paxs_recebidos(seleciona_setor, soma_vendas, df_vendas_setores_desejados, base_tm):

        if (
            (
                seleciona_setor[0]=='Transferista' and 
                st.session_state.base_luck != 'test_phoenix_noronha'
            ) or 
            (
                st.session_state.base_luck == 'test_phoenix_natal' and 
                seleciona_setor[0] in ['Desks', 'Guia']
            ) or
            (
                st.session_state.base_luck == 'test_phoenix_recife' and
                seleciona_setor[0]=='Guia'
            )
        ):
            ...
            if (
                st.session_state.base_luck == 'test_phoenix_noronha' and 
                len(base_tm)>0 and 
                base_tm[0]=='Número de Serviços'
            ):
        ...
        return tm_vendas, paxs_recebidos
    
    def calcular_ticket_medio_esperado(seleciona_setor, df_setor_meta, paxs_recebidos, df_vendas_setores_desejados):

        if (
            st.session_state.base_luck == 'test_phoenix_joao_pessoa' and 
            seleciona_setor[0]=='Guia'
        ):
            ...
            st.session_state.base_luck == 'test_phoenix_natal' and 
            seleciona_setor[0]=='Vendas Online'
        ):
    ...
    def soma_ou_media_paxs(group):
        ...
        if (
            group['Setor'].iloc[0] == 'Transferista' or 
            (
                st.session_state.base_luck == 'test_phoenix_natal' and 
                group['Setor'].iloc[0] in ['Desks', 'Guia']
            ) or
            (
                st.session_state.base_luck == 'test_phoenix_recife' and
                group['Setor'].iloc[0] == 'Guia'
            )
        ):
    ...
    def ajustando_valor_de_venda_esperado(df_vendas_esperadas):

        if (
            'Guia' in df_vendas_esperadas['Setor'].tolist() and 
        ...
        return df_vendas_esperadas

    def calculando_paxs_recebidos_tm_real_tm_meta(seleciona_setor, df_vendas_setores_desejados, soma_vendas, meta_esperada_total, base_tm):

        if (
            st.session_state.base_luck in ['test_phoenix_natal', 'test_phoenix_recife'] and
            'Guia' in seleciona_setor and
            'Transferista' in seleciona_setor and
            len(seleciona_setor)==2
        ):
        ...
        return paxs_recebidos, tm_vendas, tm_setor_estip
    ...
    if (
        len(seleciona_setor)==1 and 
        seleciona_setor[0]!='--- Todos ---'
    ):

        # Calculando Ticket Médio realizado e paxs recebidos. Dependendo da base_luck e setor escolhido pode ser diferente
    
        tm_vendas, paxs_recebidos = calcular_ticket_medio_realizado_paxs_recebidos(
            seleciona_setor, 
            soma_vendas, 
            df_vendas_setores_desejados, 
            base_tm
        )

        df_setor_meta = df_vendas_setores_desejados.groupby('Setor', as_index=False)['Meta'].first()

        # Calculando Ticket Médio esperado. Dependendo da base_luck e setor escolhido pode ser diferente
    
        tm_setor_estip = calcular_ticket_medio_esperado(
            seleciona_setor, 
            df_setor_meta, 
            paxs_recebidos, 
            df_vendas_setores_desejados
        ...
        # Em Natal só vendas online a meta é um valor nominal

        if st.session_state.base_luck == 'test_phoenix_natal':

            df_vendas_agrupado.loc[df_vendas_agrupado['Setor']=='Vendas Online', 'Meta'] = (
                df_vendas_agrupado.loc[df_vendas_agrupado['Setor']=='Vendas Online', 'Meta'] / 
        ...
        df_vendas_esperadas = ajustando_valor_de_venda_esperado(df_vendas_esperadas)
        ...
        paxs_recebidos, tm_vendas, tm_setor_estip = calculando_paxs_recebidos_tm_real_tm_meta(
            seleciona_setor, 
            df_vendas_setores_desejados, 
            soma_vendas, 
            meta_esperada_total, 
...
def gerar_df_estilizado(df_vendas_agrupado):
    ...
    def highlight_ticket(row):
        ...
        if (
            row['Ticket Médio'] > row['Meta T.M.'] and 
            row['Meta T.M.']>0
        ):
    ...
    if (
        st.session_state.base_luck == 'test_phoenix_salvador' and 
        len(seleciona_setor)==1 and 
        seleciona_setor[0]=='--- Todos ---'
    ):
        
        df_estilizado = df_vendas_agrupado.groupby(
            'Vendedor', 
            as_index=False
        ).agg(
            {
                ...
                'Servico': 'sum', 
                'Reserva': 'sum', 
                'Desconto_Global_Ajustado': 'first'
            }
        )
        ...
        df_estilizado = df_vendas_agrupado[
            [
                'Vendedor', 
                'Venda_Filtrada', 
                'Ticket_Medio', 
        ...
        df_estilizado = df_estilizado.groupby(
            'Vendedor', 
            as_index=False
        ).agg(
            {
    ...
    df_estilizado = df_estilizado.format(
        {
            'Vendas': formatar_moeda, 
            'Ticket Médio': formatar_moeda, 
            'Meta T.M.': formatar_moeda, 
            'Venda por Reserva': '{:.2f}'.format, 
            'R$ Descontos': formatar_moeda, 
            '% Descontos':'{:.2f}%'.format
        }
    )
...
def plotar_vendas_por_vendedor_setor_canal_vendas(seleciona_setor, df_vendas_agrupado, row1, i, df_vendas, df_paxs_in, df_metas_vendedor=None, df_metas_setor=None, 
    ...
    def gerar_df_estilizado_vendedor_canal_vendas(df_metas_vendedor_canal_vendas, df_vendas, df_metas_vendedor, df_metas_setor, df_paxs_in):
        ...
        df_estilizado = df_vendas.groupby(
            [
                'Vendedor', 
                'Canal_de_Vendas'
            ], 
        ...
        df_estilizado = calculando_soma_total_paxs_paxs_desc(
            df_paxs_in, 
            df_metas_setor, 
            df_estilizado
        )
        ...
        if st.session_state.base_luck == 'test_phoenix_salvador':
            ...
            df_estilizado = df_estilizado.drop(['Meta', 'Total_Paxs'], axis=1)
...
def plotar_todos_graficos_pizza_todos_setores(df_cont_passeio, row0):
    ...
    def gerar_df_todos_vendedores_filtrado(df_cont_passeio, passeios_incluidos):
        ...
        if st.session_state.base_luck=='test_phoenix_joao_pessoa':
            ...
            df_todos_vendedores_filtrado['Servico'] = df_todos_vendedores_filtrado['Servico'].replace(
                {
    ...
    if not df_todos_vendedores_filtrado.empty:
        ...
        elif st.session_state.base_luck in ['test_phoenix_joao_pessoa', 'test_phoenix_salvador', 'test_phoenix_noronha', 'test_phoenix_recife']:
...
def plotar_todos_graficos_pizza_por_vendedor(df_cont_passeio, row0):
    ...
    if st.session_state.base_luck in ['test_phoenix_joao_pessoa', 'test_phoenix_salvador', 'test_phoenix_noronha', 'test_phoenix_recife']:
...
if __name__ == '__main__':
    ...
    if not 'base_luck' in st.session_state:
        ...
        if base_fonte=='mcz':
            ...
            st.session_state.lista_colunas_numero_df_ocupacao_hoteis = [
                'Ano', 
                'Mes', 
                'Paxs Hotel'
            ]

            st.session_state.lista_colunas_numero_df_custos_com_adicionais = [
                'Valor Adicional Adt', 
                'Valor Adicional Chd'
            ]
            ...
            st.session_state.lista_colunas_numero_df_historico_vendedor = [
                'Ano', 
                'Mes', 
                'Valor', 
                'Meta', 
    ...
    if len(seleciona_setor)>0:
        ...
        if len(df_vendas)>0:
            ...
            df_cont_passeio = df_vendas.groupby(
                [
                    'Vendedor', 
                    'Servico'
                ], 
            ...
            with col3:
                ...
                elif st.session_state.base_luck in ['test_phoenix_joao_pessoa', 'test_phoenix_noronha', 'test_phoenix_recife']:
                ...
                if (
                    ...
                    if not df_setor_agrupado.empty:
                        ...
                        i = plotar_grafico_todos_setores(
                            row1, 
                            i, 
                            df_setor_agrupado
                        )

                        i = plotar_grafico_pizza_todos_setores(
                            row1, 
                            i, 
                            df_setor_agrupado
                        )
                        ...
                        i = plotar_grafico_todos_setores_vo_destrinchado(
                            df_setor_agrupado, 
                            df_vendas_vo_cv, 
                            row1, 
                            i
                    ...
                    if (
                        ...
                        i = plotar_grafico_setor_vendas_online(
                            df_vendas_agrupado, 
                            row1, 
                            i, 
                            df_vendas
                        ...
                        i = plotar_grafico_setor_especifico(
                            df_vendas_agrupado, 
                            row1, 
                            i
                        )
    ...
    if (
        ...
        plotar_todos_graficos_pizza_todos_setores(
            df_cont_passeio, 
            row0
        )
